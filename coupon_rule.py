...
class HalfOffHealthyPurchases(CouponRule):
  ...
  def can_use(balance):
    return len(balance.balance_items_with_category(Category.COUPON)) == 0 and \
      category_value(balance, Category.FRUITS) >= 20_00
...
class FiveForFive(CouponRule):
  ...
  @staticmethod
  def number_to_reward(balance):
    return 1 if balance.amount() >= 5_00 else 0
...
# Helper methods
# ---------------
def category_value(balance, category):
  valid_items = balance.balance_items_with_category(category)
